...
- Agenci są w tym repozytorium, więc workflow robi checkout `job.workflow_repository` na `job.workflow_sha` do `fix-failures/`, niezależnie od checkoutu repozytorium wywołującego do `source/`. actionlint 1.7.12 nie zna tych pól, stąd wąski wyjątek w `.github/actionlint.yaml` do czasu naprawy [rhysd/actionlint#705](https://github.com/rhysd/actionlint/issues/705). Kolejne wyjątki dotyczą nieaktualnych metadanych akcji w actionlint 1.7.12: nie znają wejścia `client-id` w `actions/create-github-app-token@v3` (a `app-id` jest tam przestarzałe) ani `permission-profile` w `openai/codex-action@v1`.
